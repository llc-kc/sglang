...
class TestHiCacheMLADedupDraftPlan(unittest.TestCase):
    ...
    def test_source_load_includes_target_and_rank_local_sidecars(self):
        ...
        self.assertIsNone(controller._prepare_draft_load(op))
    ...
    def test_source_load_and_broadcast_are_layerwise(self):
        ...
        controller._prepare_mla_peer_load = mock.Mock(return_value=None)
        controller._prepare_draft_load = mock.Mock(return_value=None)
        controller._mla_broadcast_layer_id = lambda layer_id: layer_id
        controller._packed_mtp_draft_pools = lambda: ("mtp",)
        controller._load_mla_source_layer = lambda state, layer_id: operations.append(
            ("target", layer_id)
        )
        controller._load_draft_layer = mock.Mock()
    ...
    def test_hybrid_l3_intersects_non_contiguous_restore_points(self):
        controller = HybridCacheController.__new__(HybridCacheController)
        controller.page_size = 2
        controller.prefetch_hits_sync_groups = [object()]

        def reduce_with_remote(mask, op, groups):
            remote = torch.zeros_like(mask)
            remote[[0, 4]] = 1
            mask.mul_(remote)

        controller._all_reduce = reduce_with_remote
        operation = SimpleNamespace(
            all_hash_values=["a", "b", "c", "d", "e"],
            pool_storage_result=PoolTransferResult(
                kv_hit_pages=5,
                extra_pool_hit_pages={},
                restorable_prefix_pages=[5],
            ),
        )

        # A scalar MIN would choose four pages, which the local trailing-state
        # pool cannot restore. Exact-set intersection correctly yields no hit.
        self.assertEqual(controller._sync_storage_hit_count(operation, 10), 0)
